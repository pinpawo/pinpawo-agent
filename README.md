...
- `CAPABILITY.md`, with routing metadata in frontmatter and immutable
  instructions in the Markdown body.

Most capabilities are code-free. An optional `index.js` may be declared through
the frontmatter `entry` field, but it may only export `lifecycle.finalize`.
...
export const toolkits = [
  defineToolkit({
    ...
    tools: [
      {
        tool: sampleStructuredTool,
        operation: {
          kind: 'sample.tool',
          title: 'Sample tool',
        },
        // review: optionalReviewPolicy,
      },
    ],
  }),
];
...
Capability directories are intentionally small. A minimal generated capability
looks like:
...
Use this capability as a starting point for a focused execution workflow.
```
