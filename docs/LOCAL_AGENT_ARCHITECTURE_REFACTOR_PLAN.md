...
状态：进行中。`run_shell` implementation 与 review policy 已从 `plugins/localTools.ts` 抽到 `plugins/localTools/shellTools.ts`；`http_fetch` / `download_file` 已抽到 `plugins/localTools/networkTools.ts`；`glob_search` / `grep_search` 已抽到 `plugins/localTools/searchTools.ts`；本地路径解析和文件遍历 helper 已抽到 `plugins/localTools/pathUtils.ts` / `plugins/localTools/fileSystemUtils.ts`，供 tools 共享。
