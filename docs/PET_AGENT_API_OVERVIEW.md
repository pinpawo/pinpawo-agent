...
2. **命令 API**
   ...
   - `capability` 子命令
3. **扩展协议**
   - 用户 Capability 目录（CAPABILITY.md + 可选 finalize-only index.js）
...
3. `Capability / Toolkit` 负责：可委派业务能力、Markdown instructions、
   工具权限组合与编码实现的工具能力
...
3. 想接能力接口 -> [Capability / Toolkit V2 契约](PET_AGENT_API_CAPABILITY_TOOLKIT.md#1-核心模型)
