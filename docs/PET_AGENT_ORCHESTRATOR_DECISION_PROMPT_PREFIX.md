...
> 状态：#349 Phase 2 prompt contract；capability-aware planning 已同步生产。
...
3. 有意不放入共享前缀：iteration guard、capability 候选打分细节、plan 内容。这些属于系统守卫或动态 input。

共享前缀之后，每个 decision 节点自己的段落只保留三类内容：

1. 当前阶段 + 节点边界。
2. 决策原则（节点级规则，如 entryDecision 的 execution mode 和 planner 的 task boundary）。
3. 输出 schema 指令。
