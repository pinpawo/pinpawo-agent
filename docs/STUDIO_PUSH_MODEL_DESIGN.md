...
**总原则:一切的目标是简单。** 每一条取舍的判据都是"哪个更简单",而不是
"哪个更完备"。

> 读法:§1–§2 是设计前提,§3–§5 是契约本身,§6–§7 是插件与开放问题。
> **只想知道"能不能往 studio 里加个东西"的,直接看 §5。**
...
pet 的产出经由 toolkit 落在插件自己的状态里(§4.1),与 dispatch 无关。
...
`dispatch()` 返回 `{ threadId }` 表示**已经发出去了**,不表示任务完成。

```text
提交 → 立即返回 → 客户端之后自己看插件的视图
```

契约里没有阻塞等待的入口。要知道进展,去看驱动它的那个插件的领域数据。

以上是设计前提。下面是这套前提落成的通道长什么样。
...
**所有派活必经 studio。** 插件不能绕过它直接碰 pet —— `listPets()` 只返回
纯数据 descriptor,拿不到 runtime 引用,这是机制保证而非约定。否则 pet
registry、身份与可派发性判断会在每个插件里重复一遍。

```ts
dispatch({ petId, request, correlationId? }) => { threadId }
```

`request` 是自然语言 —— studio 不定义任务结构。入参里**不带装配细节**:
pet 该有什么能力是 pet 配置的事,插件不参与 pet 的构造。

**dispatch 是点对点的。** 它只到达目标 pet:不上 event 总线(那会让每个插件
都看见谁给谁派了活,凭空制造插件间的耦合),也不进 `pet.invoke` —— pet 不
需要知道是谁派的。

studio 记录**谁派的**:插件派活时是插件名,由 studio 从它的 context 补齐 ——
插件填不了也不用填,自报的来源迟早会撒谎。外部输入则记为 `studio`。这个记录
只用于日志,不限流、不去重、不据此路由。
...
**"正忙"不在其中** —— 那正是队列存在的理由。三者都是"这条派活压根发不出
...
门当然还关着。§2.1 因此成立:studio 不认识 review,只认识门开不开。
...
宿主派的活没有回路,这是刻意的:**宿主要听,就写一个插件**(见 §6.2),而不
是给宿主开专属通道。
...
scheduler 看时间,http 插件收自己的请求),还没有哪个插件靠别人的 event
...
契约与具体插件无关。以下是第一批实现,不构成对 studio 的约束。
...
这解释了为什么宿主不需要专属的回路(§3.1):**宿主要参与,就以插件的身份
...
1. **队列不持久化,也没有上限。** 进程重启时排着队没轮到的 dispatch 直接
   ...
   倾向后者 —— studio 不持久化任何东西(§4.1),队列该守同一条。
