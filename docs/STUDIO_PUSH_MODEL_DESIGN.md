...
Tracking issue: #561 · 已随 [#629](https://github.com/pinpawo/pinpawo-agent/pull/629) 落地
...
本文是 studio 的 **canonical 设计**。`docs/` 下其余 `*STUDIO*` 文档写于拉模型
时期,描述的代码已经不存在 —— 以本文为准。

> 读法:§1–§2 是前提(是什么、为什么),§3–§5 是契约本身(通道的形状与
> 边界),§6–§9 是落地记录与待办。**只想知道"能不能往 studio 里加个东西"
> 的,直接看 §5。**
...
这是唯一的**对外协议行为变化**(已随 #629 落地)。

以上是**为什么**。下面是这套前提落成的通道长什么样。
...
拒,派活凭空丢掉 —— 而 §2.2「失败留着等人」说的是**任务失败**,不是"插板忙,
请稍后",两者不该混成同一个错误。

排队**不是业务**:它不决定派给谁(那是插件的事),只保证已经收下的派活不会
因为撞车而丢 —— 这属于**通道自身的完整性**,与 `notify` 把 event 扇给每个
订阅者是同一类事,都不涉及内容。
...
抛错 —— 三种情况都开门。这是刻意的取舍:关着的代价是**这个 pet 永久锁死**
(没有控制面能解开它),开着的代价只是退回"撞车可能并发"。宁可退化,
不可锁死。

> 推论:**没有 checkpointer 的 studio 等于没有队列保护。** `chatCheckpointer`
> 在宿主侧是可选的(#613),没配时闸门恒为 `open`,`waiting` / `blocked`
> 都不会出现。要队列真正起作用,必须配 checkpointer。
...
门当然还关着。§2.1 因此仍然成立:studio 不认识 review,只认识门开不开。
...
| gate 变化应作为 event 广播 | ❌ 同上,沿 dispatch 回发起方(§3.1) |
| 队列可以靠"`invoke` 返回了"放行 | ❌ 撞人工确认时它会提前返回(§3.1 gate) |
...
前置条件只有一个:插件持久化的落盘位置需要与 kanban 统一(§9 待定项 1)。
...
1. **队列本身不持久化,也没有上限。** 进程重启时排着队没轮到的 dispatch
   直接消失,而发起方**不会知道** —— 它的 `dispatch()` 早就成功返回了。
   同样,一个坏掉的插件疯狂派活会让队列无限增长,现在没有背压。

   这是否要修,取决于一个还没定的问题:**排队中的 dispatch 算不算"已经
   落地"的承诺?** 若算,它得跟着插件状态一起落盘;若只是尽力而为,现在
   这样就够,但得在契约里说明白。倾向后者 —— studio 不持久化任何东西
   (§4.1),队列该守同一条。
...
3. 各插件的领域模型与生命周期形态(属于插件自身,不进本契约)。
4. event 是否需要"至少一次"投递保证。目前是纯内存同步扇出,进程重启即丢。
   ...
   **暂时没有理由需要。** 到目前为止每个插件都是自足的:kanban 听自己的
   ...
   保证是凭空的复杂度。
5. 插件的 `start` 顺序即配置顺序,`stop` 逆序。若先启动的插件在 `start` 里
