...
1. **输出收缩**：旧的大工具输出通过 contextPolicy 收缩，触发信号由 `ProviderUsageWatermarkGuard` 读取 provider 返回的 `usage_metadata.input_tokens`，不用本地 token 估算做极限信号。
...
| 设计点 | 现有机制（代码） | 状态 | 缺口 |
|---|---|---|---|
| 1 上下文体积 | 旧实现使用本地 token 估算 fuse | ✅ 已替换 | 不再维护本地 token 估算；主线 compact 和能力级工具输出收缩都通过 `ProviderUsageWatermarkGuard` 判断 provider `usage_metadata.input_tokens` 水位，真实超限仍由 provider 错误兜底 |
...
- **ProviderUsageWatermarkGuard** → 判断最近一次 provider input_tokens 是否过 budget 阈值；只给 verdict，不执行压缩。
- **contextPolicy（evict/truncate）** → 仍是压缩执行器，不是 Guard/Decision；只消费水位 guard verdict 后执行。
...
3. **P3 删除本地 token fuse**：不再把本地 token 估算表达成 Guard；输出体积靠 provider usage metadata guard 触发的 contextPolicy 和工具边界治理。
