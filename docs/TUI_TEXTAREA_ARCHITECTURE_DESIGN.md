...
> 状态：Draft v2 / implementation-aligned
> 日期：2026-06-18
...
截至 `codex/tui-textarea-refinements-bundle`，本文的核心结构已经落地：
...
Ink useInput
  ...
  -> canonicalInput.ts
  -> inputRouter.ts
  -> textarea/commands.ts
  -> textarea/engine.ts
  -> textarea/layout.ts
  -> textarea/renderModel.ts
  -> textarea/viewModel.ts
  -> useTextAreaController.ts
  -> Composer.tsx / TextAreaView.tsx
```

当前代码已经覆盖 terminal decoder、canonical mapper、owner/focus router、textarea command、engine/layout/render/view model、controller、prompt history、selection、undo/redo、preferred column、grapheme-aware editing 和 approval free-text routing helper。

剩余工作主要不是继续拆结构，而是：

- 清理 legacy raw input wrapper，只允许生产路径使用 `TextAreaCommand`。
- 明确 approval free-text 的 focus policy。
- 增加 command popup、file mention / path search、optional external editor 等高阶输入能力。
- 继续用本文的 conformance matrix 固化新发现的 terminal/input bug。
...
生产路径禁止新增或调用这样的 raw input 接口：

```ts
applyTextAreaInput(input: string, key: TuiKeyInput, state)
```

当前代码仍保留 `applyTextAreaInput` / `applyTextAreaInputEvent` 作为测试和旧 facade 兼容入口。它们不应被 `TuiApp`、`inputRouter` 或新功能继续依赖；后续清理时应把 raw wrapper 移出 engine 主边界或删除。
...
- textarea 生产主入口是 `applyTextAreaCommand`，不依赖 Ink key shape；legacy raw wrapper 只能作为兼容尾巴存在，不可扩展。
...
下面是 textarea 重构的原始切分和实现顺序。当前聚合栈已经覆盖 1-34；后续新工作应从 35 或本文 §1.1 的剩余项继续，不要重新按单个按键 bug 开散 PR。
...
18. `codex/tui-composer-history-routing`
   ...
   - `TuiApp` 先接入 boundary 上下文，但保持 history availability disabled，不改变当前
     上下键行为。
19. `codex/tui-composer-history-state`
   - 新增 composer prompt-history state helper。
