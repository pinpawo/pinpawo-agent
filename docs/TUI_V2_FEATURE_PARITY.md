...
| runtime | interrupt, error notice, review policy | aligned | production interruption, graph-failure recovery, immediate stop wake-up, and process-level host restart are covered; if the authoritative interrupt result remains pending for ten seconds, v2 escalates its local notice without fabricating completion or unlocking input; full CLI/browser-runtime dogfood remains |
| status | immediate waiting feedback plus two-line run/model/workspace/token/context status | aligned | accepted requests immediately pulse with the canonical session actor before the first delta, distinguish tool/stream/review/interrupt states, leave the composer available for drafting, and stop idle animation so terminal-native history remains browsable; a quiet phase gets one “still working” refresh after ten seconds instead of a continuously repainting elapsed clock; explicit `pinpawo tui --v2 --qa` now drives the normally resolved production UI/bundle through the full deterministic lifecycle with predictable final usage in any terminal without requiring a host or model account; narrow-terminal dogfood remains |
