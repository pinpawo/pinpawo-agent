...
| production review resolution | canonical review interrupts reach v2 as waiting-review state; approval and rejection traverse the authenticated host, while cancellation interrupts and suspends the active delegation instead of fabricating a rejection; after the matching session authoritatively reports `interrupted`, `/continue <guidance>` sends `resume_active`, reopens the exact review, and only then permits approval and tool execution; one no-smoke PTY drives a direct LangGraph checkpoint interrupt through the visible overlay, while a second production graph proves cancellation and continuation remain side-effect free until the reopened review is approved, then settles the operation before the final reply and restores canonical messages without synthetic delegation-plan leakage after process restart | Bun native + child-process production PTY test |
...
| review continuation ownership | `/continue` is hidden until a tracked review cancellation receives the matching authoritative interruption, remains scoped to that session across session switches, survives transport failure, and is consumed only by an accepted `resume_active` request | automated + Bun native + child-process production PTY test |
...
| unit tests | `npm run test -w @pinpawo/tui` | passed, 125 tests |
...
| root tests | `npm test` | passed on macOS and after a clean Linux arm64 `npm ci`, including local-agent 787/787 and Chrome extension 22/22 |
...
| installed tarball v2 startup | `npm run test:tui-install -w pinpawo` installs generated local tarballs in an empty project and runs the installed `pinpawo tui --v2 --check` path | passed on darwin-arm64 and Linux arm64 with package-local Bun/OpenTUI runtime; interactive PTY remains separately covered |
