...
| review continuation ownership | `taskActiveDelegation` is the sole authority: snapshots project its presence as `hasResumableDelegation`, `/continue` is visible only while that focused session is idle, and client-local cancellation history neither grants nor revokes it; accepted `resume_active` or `supersede_active` input hides the affordance behind the active run until the next authoritative snapshot | automated + Bun native + child-process production PTY test |
