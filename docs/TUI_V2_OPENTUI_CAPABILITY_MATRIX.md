...
| production TUI process entry | the real no-flag `main.ts` process runs inside an expect-managed macOS PTY, authenticates to an independent production host, applies its snapshot, accepts attachment paste/removal, composer/history/`@path` flows, palette/help ownership, and a palette-completed `/edit` that suspends the renderer for a real `$VISUAL` child before restoring and submitting its Unicode multiline draft; it exits through palette-completed `/quit` and leaves an ordered checkpoint that a restarted host rehydrates with cumulative token usage | Bun child-process PTY integration test |
...
| focused process lifecycle | `npm run test:process -w @pinpawo/tui` | passed; production hosts checkpoint, restore, and continue turns; the no-smoke PTY covers selected attachments, prompt history, Unicode workspace mention, help ownership, a real `$VISUAL` handoff with multiline draft submission, and palette-completed `/quit`; all five turns plus cumulative usage survive host restart, and spawn failures settle without hanging |
