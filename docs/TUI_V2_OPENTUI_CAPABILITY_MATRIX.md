...
| production multi-path paste | the production attachment reducer and OpenTUI paste callback separate quoted multi-path input into distinct attachments, keep path text out of the composer, preserve ordinary multiline paste, and report duplicates/protocol limits; a real macOS PTY pastes three paths with spaces and Unicode, removes the last with Backspace, and proves only the selected paths reach model input while screen/checkpoint output remains filename-only across host restart | unit + Bun native + child-process PTY test |
...
| production TUI process entry | the real no-flag `main.ts` process runs inside an expect-managed macOS PTY, authenticates to an independent production host, applies its snapshot, accepts attachment paste/removal, composer/history/`@path` flows, renders an updated operation followed by distinct subagent progress and multi-delta Markdown, resolves a real checkpointed review through the approval overlay, and performs real `$VISUAL` plus `$PAGER` handoffs; after pager return it exports the same canonical messages, exits through palette-completed `/quit`, and leaves an ordered checkpoint that a restarted host rehydrates with cumulative token usage | Bun child-process PTY integration test |
...
| external editor lifecycle | `/edit [text]` suspends OpenTUI, gives the TTY to `$VISUAL`/`$EDITOR`, resumes on success or failure, and restores the multiline draft without submitting; the production process then submits that exact restored draft through the host | automated + smoke PTY + child-process production PTY test |
| keyboard transcript browsing | PageUp from an empty composer or `/transcript` hands the full ordered canonical timeline to `$PAGER`, buffers projection rendering while suspended, and reconciles after return; the production PTY uses a real child to verify all seven ordered turns plus operation/subagent rows, filename-only attachments, and restored composer ownership | automated test + interactive pager PTY + child-process production PTY test |
| transcript export | `/export [path]` writes completed canonical user/assistant messages locally without a new host protocol or local-agent implementation import; the production PTY reads the resulting Markdown and verifies message order plus local-path privacy | automated + child-process PTY test |
...
| focused process lifecycle | `npm run test:process -w @pinpawo/tui` | passed; production hosts checkpoint, restore, and continue turns; the no-smoke PTY covers selected attachments, prompt history, Unicode workspace mention, ordered operation/subagent/streaming-Markdown rendering, a real checkpointed approval and keyed resume, help ownership, real `$VISUAL` and `$PAGER` handoffs, Markdown export, and palette-completed `/quit`; all seven user/assistant turns plus cumulative usage survive host restart, while transient operation/subagent/review UI state does not become checkpoint messages and spawn failures settle without hanging |
