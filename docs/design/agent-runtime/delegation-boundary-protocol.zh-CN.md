...
行为测试覆盖控制工具一次调用、消息证据、续接、计划约束、压缩保留和 checkpoint 恢复。真实模型 eval 使用虚构场景；2026-09-07 在 DeepSeek v4-pro 上最新一轮 8 个场景通过 7 个，包含自然询问和两段用户回答后的续接。收尾场景输出 XML 文本而未调用控制工具，保留为失败。单轮结果不代表稳定性已经验证。
