...
Tools，也不由 Agent 中间件手工执行 Tool。断线中的调用报告未知结果；后续新调用可用
新 clientId 重连，旧 handle 失效，不重放断线时的调用。
...
- 服务进程只接收允许列表中的基础 OS 环境变量，不继承启动 Host 从项目 `.env` 载入的
  密钥。Shell 实例需要的额外变量由服务配置中的 `env` 显式提供；配置与启动环境变更
  仍需重启服务。实例初始化失败后短暂退避，下一次请求可以重试。
...
| 情况 | 行为 |
| --- | --- |
| 初始化时连接、协议或绑定失败 | 依赖服务的 Toolkit 标记为 unavailable；纯 Host Toolkit 继续可用，不在 Host 执行 Runtime fallback。修复后重启 Host |
| 操作取消 | 请求先登记再启动异步工作；处理取消竞态，确认前不声称已终止 |
| 输出过多 | 有界缓存、游标续读与显式截断，保留同一连接内的进程输出能力 |
| 响应丢失或连接断开 | 未知结果明确报告；清理本 client 资源，不自动重放有副作用的操作 |
| 后续调用触发重连或服务重启 | 新 client 身份；旧进程/页面 handle 失效，不能接管旧任务 |

Host 在后续调用时惰性重连，不重试结果未知的原调用。本期不做心跳租约、断线续传、
去重账本或旧任务恢复。连接内 yield 与输出续读属于已有工具能力；连接断开后继续持有任务
则属于后续恢复设计。CDP 自身的连接/页面
