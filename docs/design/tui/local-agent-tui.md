...
> **范围边界。** studio 的完整链路——UI Role Boundary、Turn State Stream、ws 协议、planner/dispatch/wiki、pet 调用契约——定义在 `../../history/studio/legacy-orchestrator.md` 与 `../../history/studio/interfaces.md`；事件归一化与 stream→event 映射定义在 `../local-agent/architecture-refactor.md` §5。本文档**不**复制或定义这些链路，只保证 TUI state 形状为它们留出空间（见 §5 的 session-keyed 模型）。
