...
| embedded stdio host transport | `pinpawo tui --embed-host` starts its own local agent as a piped stdio child and drives the same session controller over the shared JSONL protocol with no port, bearer token, or loopback origin check; the Host's stdout carries frames only, its stderr is appended to `~/.pinpawo/logs/embedded-host.log`, disconnecting ends the child, and the synchronization timeout is disabled because this transport cannot re-dial a running Host | unit + Bun child-process integration test |
