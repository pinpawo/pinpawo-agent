...
这对只返回小型结构化结果的任务足够，但对 `explore`、媒体生成、文件产出类 capability 不够：
...
- 结构化结果 capability 不再从消息里抓 `ToolMessage.artifact`。
...
- 结构化结果 capability：通过 `capability_artifact_write` 保存 JSON result artifact。
