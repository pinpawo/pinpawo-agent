...
>
> Current flow: [Capability Artifact Pipeline](../../reference/artifacts/architecture.md).
...
- Backend-owned asynchronous media that returns a CDN/object-storage URL should
  write `externalUri` and should not
...
- A structured-result Capability persists its result in an `afterRun` middleware:
  it takes the latest schema-valid tool artifact and writes it as a
  `kind: "result"` artifact via `ctx.artifactStore`. No model-facing write tool
