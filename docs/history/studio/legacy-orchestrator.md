...
> Status: legacy design. The Studio runtime is being redesigned without compatibility requirements; use `../../studio/run-controller.md` as the canonical design and `../../studio/run-controller-iteration-plan.md` as the iteration plan.
...
架构整体心智模型见 `architecture-overview.md`。核心立场:
...
Studio↔pet 的接口契约(invoke 签名、wiki middleware、HITL 边界等)见 `interfaces.md`。Capability artifacts 的 durable store 见 `../../reference/artifacts/store.md`。
...
`ToolMessage.artifact` 仅是 capability 内部临时回执桥,不是 durable store。详细 contract 见 `../../reference/artifacts/store.md`。
