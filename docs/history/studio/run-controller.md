...
> **Status: historical.** This document describes the removed pull-model
> `StudioOrchestrator`. It is retained for migration context only; use the
> current [Studio push model](../../studio/index.md) instead.

读者：Studio 运行时与集成开发者。首次了解多 agent 模型时，请先阅读
[Core Concepts](../../concepts/core-concepts.md)和[Architecture](../../concepts/architecture.md)。
