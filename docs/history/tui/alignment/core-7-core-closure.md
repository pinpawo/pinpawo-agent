...
| 2026-06-23 | non-checkpoint reducer coverage | system feedback and Studio progress should be reducer-owned notices, not checkpoint timeline messages. | Existing coverage proved the shared system-cell adapter and terminal error/Studio notices, but did not directly cover live `system.notice` and `studio.progress` event dispatch. | CORE-7 moved these surfaces out of timeline, so the live reducer path should be protected explicitly. | Add a reducer test proving both event types append notices and leave timeline/transcript unchanged. | none | accepted |
