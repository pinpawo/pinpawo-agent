...
- Artifact 引用与 Studio dispatch 是不同边界。当前 Studio 只在接收 dispatch 时
  返回 `threadId`，不持有或返回“最终 worker / dispatch”身份。
...
| Studio | 多 Pet dispatch 底座 | 不拥有 artifact、任务结果或共享知识存储 | 进程级 |
...
- `finalDispatchId` 不是当前 Studio 或 artifact API 的字段；旧文档里的该名称属于已移除的 pull-model。
- 如果看到文档里出现 `artifactId` 或 `invoke(...).artifacts`，均为过时表述；请以本页和实际类型定义为准。
