...
- 无法解析的 patch 也会确定性授权：执行器使用同一个 parser，并保证在任何文件写入前返回结构化解析错误；模型收到错误后自行修正下一次调用。
- 目标位于 workdir 外、经过符号链接逃逸、文件不存在，或同一批次还包含未通过确定性策略的操作时，仍走原有 auto-review；信息不足或风险较高时继续要求人工审核。
- `apply_patch` 不建立 session matcher。每次调用都重新检查当前 workdir、真实目标路径和文件类型，避免旧授权绕过当前文件系统边界。
