...
| 2026-06-23 | StatusBar resume mode coverage | `/studio -> /chat -> /resume` should leave submit mode, session kind, and status bar coherent. | Existing coverage proved Studio submit mode wins over a chat session kind, but did not cover the inverse resumed-Studio-session state where `session.kind` remains `studio` while the reducer-owned submit mode and status bar are `chat`. | Main design lists this sequence in the minimum test matrix, and resume snapshots now preserve server-provided session kind while clearing stale submit mode. | Extend the status bar model test to cover both mismatch directions between submit mode and session kind. | None. | Done |
...
| 2026-06-23 | Overlay/input owner priority | The visible overlay owner and routed input owner should stay aligned for modal and popup states. | Overlay priority and input owner priority were tested separately, but no test bound the two orderings together. | Main design treats overlay and input owner as one UI ownership boundary; separate tests can drift when a new overlay is added. | Add a regression test that compares `TuiOverlayModel.owner` with `resolveTuiInputOwner(...).type` across resume, approval, policy, command, and file owners. | None. | Done |
...
- [x] Status bar mode is covered for both submit-mode/session-kind mismatch directions.
...
- [x] Overlay owner priority is regression-tested against input owner routing.
