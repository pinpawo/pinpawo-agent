...
| 2026-06-23 | StatusBar tones | Segment tone should be visible in the rendered status bar. | `StatusSegment.tone` existed in the model, but `BottomStatusLine` rendered the entire line as dim text; reconnecting status was also easy to classify the same as disconnected. | Warning, danger, success, and info states are part of the structured status model and make busy/reconnect/disconnected states clearer. | Format visible status parts with segment tone metadata and render each part with the matching Ink color while preserving existing text truncation behavior; classify retry/reconnect as warning and disconnected/unavailable/failure as danger. | None. | Done |
