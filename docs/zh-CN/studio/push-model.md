...
配置：[Studio 配置](configuration.md)

本文说明当前 Studio 的 canonical 推模型。旧的拉模型编排器、run controller、
due-run scheduler 与 shared wiki 文档均已移至 `history/studio/`，仅供理解演进，
不定义当前行为（见附录）。
...
仅此而已。插件通过 `StudioPluginContext.subscribe` 使用这条总线；宿主也可使用
公共 `Studio.subscribe` 建立传输事件桥，但必须自己处理事件的完整字段和可靠性。
...
当前 local-agent 的 WebSocket handler 是宿主适配器：它直接调用 public
`Studio.dispatch` 并订阅 public event。若新入口需要自己的领域状态或 gate 回路，
应实现为插件，而不是把该领域概念加入 Studio。
...
`history/studio/` 中的 [legacy orchestrator](../../history/studio/legacy-orchestrator.md)、
[run controller](../../history/studio/run-controller.md) 与
[due-run scheduler](../../history/studio/due-run-scheduler.md) 描述的是一套
**拉模型编排器**：Studio 派活后等结果、持有 run/task 状态机、自动重试、超时判定。
那套实现已移除，相关文档仅作历史记录。
