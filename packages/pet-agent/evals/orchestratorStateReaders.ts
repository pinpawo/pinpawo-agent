...
export function readPendingDelegation(result: EvalOrchestratorStateSnapshot) {
  if (!result.messages || !result.runId || !result.taskId || !result.runSupervisorState) return null;
  try {
...
export function readRunDelegationSummaries(result: EvalOrchestratorStateSnapshot) {
  ...
  return parsed.data.plan.flatMap((task) => {
    const latest = executionsForPlanItem({ messages: result.messages ?? [] }, task.id)
      .filter(({ metadata }) => metadata.runId === result.runId).at(-1);
    if (!latest) return [];
    const delivery = latest.result?.delivery;
    return [{ id: latest.execution.delegationId, lane: `capability:${task.capability}` as const, task: task.objective,
...
export function readTaskActiveDelegation(result: EvalOrchestratorStateSnapshot) {
  ...
  if (!task) return null;
  const latest = executionsForPlanItem({ messages: result.messages ?? [] }, task.id).at(-1);
  if (!latest) return null;
  return { id: latest.execution.delegationId, lane: `capability:${task.capability}` as const,
    task: task.objective, contextSummary: latest.execution.briefing, runId: String(latest.metadata.runId),
    taskId: String(latest.metadata.taskId), status: latest.result?.status === 'returned' ? 'awaiting_decision' : 'pending',
