...
test('answer filters private delegation transcripts by lane without parsing message text', async () => {
  let answerInput = '';
  const model = {
    invoke: async (messages: unknown[]) => {
  ...
  const privateTranscript = new AIMessage('正文完全普通，但 metadata 表明它属于 delegation lane。');
  setAgentMessageMetadata(privateTranscript, {
    lane: 'capability:general',
    runId: 'answer-run',
  ...
  assert.equal(canonical.includes(privateTranscript), false);
});
...
test('limit-reached progress announce lets model choose the same capability delegation', async () => {
  ...
  setAgentMessageMetadata(progressAnnounce, {
...
test('buildSubagentHandoff copies the announce into main and wipes the whole delegation lane', () => {
  ...
  setAgentMessageMetadata(announce, { lane: 'capability:explore', runId: 't1', delegationId: 'd1', isAnnounce: true, task: '查动态' });
...
test('buildSubagentHandoff retains only announce identity and result', () => {
  ...
  setAgentMessageMetadata(announce, {
    lane: 'capability:explore',
    runId: 'run-1',
...
test('Planner continue_current action can re-enter main and finalize handoff', async () => {
  ...
  setAgentMessageMetadata(previousAnnounce, {
    lane: 'capability:general',
    runId: input.runId,
    delegationId: activeDelegation.id,
    isAnnounce: true,
...
test('Planner boundary accepts each announce attempt once', async () => {
  ...
  setAgentMessageMetadata(initialAnnounce, {
    lane: 'capability:general',
    runId: input.runId,
    delegationId: activeDelegation.id,
    isAnnounce: true,
...
test('Planner boundary does not handoff a limit_reached announce', async () => {
  ...
  setAgentMessageMetadata(partialAnnounce, {
    lane: 'capability:general',
    runId: input.runId,
    delegationId: activeDelegation.id,
    isAnnounce: true,
...
test('explicit resume reuses checkpointed delegation identity and ToolMessages', async () => {
  ...
  setAgentMessageMetadata(priorAnnounce, {
    lane: activeDelegation.lane,
    runId: activeDelegation.transcriptRunId,
    isAnnounce: true,
    completionReason: 'natural',
  ...
  assert.equal(
    getAgentMessageTranscriptRunId(resumedBriefing),
    null,
  );
  assert.equal(
    resumedInput
...
test('delegation briefing stays invocation-scoped across sequential tasks', async () => {
  ...
  // selected canonical main and delegation transcript.
