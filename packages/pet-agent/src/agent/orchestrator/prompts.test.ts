...
test('entry decision prompt owns execution mode selection', () => {
  ...
  assert.match(prompt, /判断是否必须先 plan/);
  assert.match(prompt, /准备、操作、验证、汇总和同类批量处理/);
  assert.match(prompt, /内部动作可以使用前面动作的结果/);
  assert.match(prompt, /一个 task 完成后仍有需要单独执行和验收的 task/);
  assert.match(prompt, /后续 task 的内容必须等待前一个 task 的结果/);
  assert.match(prompt, /选择 needs_plan/);
