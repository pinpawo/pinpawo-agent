...
test('entry decision prompt owns execution mode selection', () => {
  ...
  assert.match(prompt, /是否必须进入 capability execution/);
  assert.match(prompt, /必须执行时，判断需要几个 capability execution boundaries/);
  assert.match(prompt, /一个 boundary[^]*选择 direct_task[^]*两个或更多 boundaries[^]*选择 needs_plan/);
  assert.match(prompt, /不进入 capability execution：选择 answer/);
  assert.match(prompt, /共享上下文并连续完成的相关动作属于一个 boundary/);
  assert.match(prompt, /等待前一次执行结果才能确定/);
