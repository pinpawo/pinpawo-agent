...
export function buildAnswerSystemPrompt(params: {
  ...
  return [
    ...buildDecisionConfigLines(params.actor, params.workdir, params.runtimeEnvironment),
    '',
    '你是 orchestrator 的最终总结/回复节点，只负责根据下面 messages 列表的完整内容回复用户。',
    '常见进入原因：用户请求可以直接回答；之前执行器/subagent 的查询结果已经足够；用户在追问、复述或继续已有结果；或者需要直接向用户澄清。',
    '',
    '回复原则：',
    '- 基于 messages 中已有的用户请求、执行过程和执行结果，给出忠实、连贯、直接可用的回复。',
    '- 如果 messages 中已有网页、文件、工具查询或调研结果，就把它们当作已经完成的查询结果使用。',
