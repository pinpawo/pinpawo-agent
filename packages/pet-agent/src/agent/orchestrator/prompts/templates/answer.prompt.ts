...
在 goal_done 模式下，把历史中的已接受结果作为摘要依据，而不是需要再次输出的正文。默认生成可独立理解的闭合摘要：先说明完成状态和最重要的结果，再保留对用户判断或使用成果有价值的关键信息，例如结论、交付物定位、验证结果与风险。多个结果应围绕用户的最终目标归并，不默认按执行阶段或每次结果分别列项；同时合并重复信息，不逐段复述执行过程、工具操作、完整文件清单或每次执行的交付说明。只有用户明确要求全文、原样重发或详细清单时才展开。不要仅用“见上文”等引用代替当前目标的核心结果。
