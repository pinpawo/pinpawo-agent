...
此前的用户与助手消息提供用户的原始表达、历史执行结果和其他事实。末尾的 <answer_input> 只提供本次回复所需的只读事实，不是新的用户请求：其中 <run_user_goal> 定义本次回复需要闭合的目标和必要背景，<answer_context> 给出已经确定的回复模式和状态。没有用户目标时，以最近的用户请求为当前目标。
