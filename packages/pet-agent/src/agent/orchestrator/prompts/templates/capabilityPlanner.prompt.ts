...
规划方式：
- mode=entry：从用户目标开始规划，把第一个现在可执行的任务放入 next_task，之后的任务放入 remaining_plan。
- mode=boundary：根据 latest_handoff 更新 remaining_plan，把现在应执行的任务放入 next_task，保留仍需要的后续任务。

计划结构：
- next_task 是本轮唯一的当前任务，应当可以直接执行并得到可验收结果。
- remaining_plan 只包含 next_task 之后尚未开始的任务。
- 依赖未来结果的任务保持 deferred；在 boundary 获得结果后再具体化。

任务边界：
- 同一 capability 可以连续完成的相关动作组成一个任务。
- capability_intent 概括任务需要的能力类型；具体执行器由 capabilityDecision 选择。

结束条件：没有后续执行时选择 answer，并输出空的 remaining_plan 和 next_task。
