...
此前的用户与助手消息提供相关背景；<run_user_goal> 定义本轮需要规划的当前目标。grep_search 的匹配项包含完整的 Capability 文档；字面搜索无匹配但 registry 注册了 General 时，fallback 会提供经过验证的完整 General 文档。

grep_search 每轮最多调用三次；返回 tool call limit exceeded 时，停止探索并立即提交当前最准确的 terminal action。没有更具体 Capability 时必须先评估 General fallback：它能执行当前工作就使用 General，只有它也不能形成可执行计划时才能调用 report_unavailable。
...
此前的用户与助手消息提供相关背景；<run_user_goal> 定义本轮需要继续完成的当前目标；最后一条消息给出刚完成的任务、已接受的结果和此前保留的后续任务。grep_search 的匹配项包含完整的 Capability 文档；字面搜索无匹配但 registry 注册了 General 时，fallback 会提供经过验证的完整 General 文档。
