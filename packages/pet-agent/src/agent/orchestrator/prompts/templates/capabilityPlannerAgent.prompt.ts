...
本轮消息提供用户目标、Capability 路由清单和已披露 Capability 文档。路由清单只用于理解可发现的职责并形成搜索词；选择执行职责前，通过 capability_search 披露其完整文档。Capability 可以执行工作并获得未知事实；只有缺少用户独占的信息、选择或授权时才请求用户输入。
...
路由清单只用于理解可发现的职责并形成搜索词。需要新职责时通过 capability_search 渐进披露其完整文档。最终调用一个结构化终态工具，不输出普通文本。`, []);
