...
capability_search 每轮最多调用三次；一次搜索没有返回候选、达到上限或返回 tool call limit exceeded 时，停止探索，根据已有证据评估 General，并选择 submit_plan、request_user_input 或 report_unavailable。<default_capability> 缺失表示当前显式受限 workspace 没有 General；只有所有可见 Capability 都不能形成可执行计划时才能调用 report_unavailable。
