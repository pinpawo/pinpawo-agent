...
动态上下文内容：
- entry_decision_context：本次调用的运行环境、压缩摘要、artifact 短引用和当前 run state，仅作为事实背景。
- entry_decision_context 之后的原生 main messages：用户请求、assistant 回复和 handoff 结论；保持真实角色与时间顺序，是理解用户指代和目标的唯一对话来源。
...
export const ENTRY_DECISION_INPUT_PROMPT = definePromptTemplate<{
  ...
  compactionContextBlock: string;
  capabilityArtifactsBlock: string;
  runDelegationContextBlock: string;
}>(`<entry_decision_context>{runtimeContextBlock}{compactionContextBlock}{capabilityArtifactsBlock}{runDelegationContextBlock}
