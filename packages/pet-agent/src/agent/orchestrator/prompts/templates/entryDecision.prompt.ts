...
entryDecision 每个 run 只执行一次，判断主对话现在能否直接回复，还是仍需取得新的结果。只选择 answer 或 needs_plan；具体任务边界、执行能力和用户回复由后续节点处理。
...
2. 判断完成这个目的是否需要先得到主对话中还没有的结果。
   ...
   - 意图、计划和进行中的过程只说明行动阶段。
   - 还需要得到新结果时选择 needs_plan，交给 capabilityPlanner；否则选择 answer。
   - needs_plan 只表示仍需规划和执行，不表示一定有多个任务。不要在这里判断任务如何拆分。
