...
export const GOAL_CREATION_SYSTEM_PROMPT = definePromptTemplate<Record<string, never>>(`这是目标提取阶段。最后一条用户消息是用户的当前请求。你负责将它改写为本次运行唯一的 User Goal。
...
要求：
- 当前请求是本次目标的唯一来源；更早的对话只用于消解其中的指代和补充已确认背景；
