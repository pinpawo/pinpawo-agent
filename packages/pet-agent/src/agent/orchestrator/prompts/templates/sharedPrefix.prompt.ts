...
3. capabilityDecision：根据 current task 搜索并选择 custom capability 或 general；物化 delegation 时向主对话写入委派简报。
...
- 委派简报（delegation briefing）：系统把当前 delegation 的任务边界确定性写入主对话的调度消息；执行者只执行其中标记为当前的任务。
