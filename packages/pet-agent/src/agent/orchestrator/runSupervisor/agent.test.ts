...
class ScriptedSupervisorModel extends BaseChatModel {
  ...
  _llmType() {
    return 'scripted-run-supervisor';
  }
...
async function createWorkspace(
  ...
  const rootPath = await mkdtemp(join(tmpdir(), 'run-supervisor-agent-'));
...
test('Supervisor reports closed discovery after two rounds while keeping search auto', async (t) => {
  const workspace = await createWorkspace(t, {
    general: capabilityDocument({
      name: 'general',
      description: 'Handle ordinary workspace tasks.',
...
test('Supervisor receives verified General before discovery starts', async (t) => {
  const workspace = await createWorkspace(t, {
    general: capabilityDocument({
      name: 'general',
  ...
  assert.match(readMessageText(supervisorInputMessage), /通用工具读取和修改工作区/);
...
test('Supervisor receives the configured default Capability before discovery', async (t) => {
  const workspace = await createWorkspace(t, {
    general: capabilityDocument({
      name: 'general',
  ...
  assert.match(readMessageText(supervisorInputMessage), /create Kanban tasks/);
...
test('Supervisor reports a no-command result when it exits without a command', async (t) => {
  const workspace = await createWorkspace(t, {});
  const model = new ScriptedSupervisorModel([{
