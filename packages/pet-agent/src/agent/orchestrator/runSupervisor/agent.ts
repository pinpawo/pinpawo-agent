...
import {
  projectRunSupervisorMessagesForModel,
} from './messageContext';
...
export function createRunSupervisorAgent(params: {
  model: BaseChatModel;
  /** Capability preloaded as the entry Supervisor's default candidate. */
  ...
  const agent = createAgent({
    ...
    middleware: [middleware, createSupervisorSearchStateMiddleware()],
    checkpointer: false,
  });
    ...
    async invoke(
      ...
      try {
        ...
        const result = await agent.invoke({
          messages: [
            ...projectRunSupervisorMessagesForModel(input.messages),
            new HumanMessage({
