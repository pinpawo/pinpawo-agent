...
async function workspaceFixture(
  ...
  const root = await temporaryDirectory(t, 'capability-supervisor-files-');
...
test('Supervisor can read any disclosed Capability and search every other document', async (t) => {
  const { workspace } = await workspaceFixture(t);
  const explorer = createRunSupervisorFileExplorer({ workspace });
