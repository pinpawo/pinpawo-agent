...
import type { ToolReviewInterruptPayload } from '../orchestrator/review/reviewSpec';

export type HumanReviewerRequest = HumanReviewRequest | ToolReviewInterruptPayload;

/**
 * HumanReviewer:pet runtime 在 invoke 期间撞到 HITL interrupt 时回调的桥。
 *
 * pet runtime 不关心上层是 ws / SSE / TUI / 进程内 mock,只承诺:
 * - 给出 legacy `HumanReviewRequest` 或 canonical `ToolReviewInterruptPayload`
