...
>(options: HumanReviewResolutionOptions<TRoute>) {
  ...
  try {
    ...
    // turned out to be unusable — in both cases the next read of the
    // checkpoint decides what, if anything, is still pending. An early return
    // (stale action, wrong session, disconnected peer) and an ordinary failure
    // both keep the route, since resolving it again is still meaningful.
