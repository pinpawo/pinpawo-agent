...
test('chat mode rejects studio requests instead of running them', async () => {
  // #561:mode 是执行边界,不只是启动预检与投影。
  const workdir = mkdtempSync(join(tmpdir(), 'pinpawo-mode-chat-'));
  const sent: LocalAgentServerMessage[] = [];
  const peer: LocalServerPeer = {
    isConnected: () => true,
    send: (message) => {
  ...
  await handlers.peerHandlers.onStudioRequest(peer, {
    type: 'studio_request',
    requestId: 'studio-wrong-mode',
    userRequest: 'should not run',
  });

  // 必须显式拒绝而不是静默丢弃,否则客户端会一直等一个不来的回包。
  const error = sent.find((message) => message.type === 'studio_error');
  assert.ok(error, 'expected a studio_error reply');
  assert.match(
    (error as { message: string }).message,
    /chat mode; studio requests are not accepted/,
  );
  assert.equal(sent.some((message) => message.type === 'studio_response'), false);
});

test('studio mode rejects chat requests instead of running them', async () => {
  const workdir = mkdtempSync(join(tmpdir(), 'pinpawo-mode-studio-'));
  ...
  await handlers.peerHandlers.onChatRequest(peer, {
    type: 'chat_request',
    requestId: 'chat-wrong-mode',
    message: 'should not run',
  });

  const errorEvent = events.find((message) => {
    const envelope = message as { type?: string; event?: { type?: string } };
    return envelope.type === 'event' && envelope.event?.type === 'error';
  }) as { event: { message: string } } | undefined;
  assert.ok(errorEvent, 'expected an error event');
  assert.match(errorEvent.event.message, /studio mode; chat requests are not accepted/);
});
