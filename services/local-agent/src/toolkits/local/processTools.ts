...
import { shellInvocation, rethrowAbort, type ShellProcess } from './shellClient';
 ...
 * timeout.
 *
 * Static tools read their client's identity from the current invocation.
 * Process handles and registry state remain in the service.
 */
...
function describeStatus(record: ShellProcess) {
...
export function createProcessTools(): StructuredTool[] {
  ...
  const waitProcessTool = tool(
    async (
      ...
      try {
        ...
        const result = await scope.client.wait({ processId, timeoutMs: seconds * 1000 }, scope);
  ...
  const terminateProcessTool = tool(
    async ({ processId }: { processId: string }, runtime: ToolRuntime) => {
      try {
        const scope = shellInvocation(runtime);
        const record = await scope.client.terminate(processId, scope);
  ...
  const listProcessesTool = tool(
    async (_input: Record<string, never>, runtime: ToolRuntime) => {
      const scope = shellInvocation(runtime);
      const records = await scope.client.list(scope);
...
export const processTools = createProcessTools();
