...
test('a closed target rejects late events instead of resurrecting', () => {
  const registry = new ManagedTargetRegistry();
  const target = registry.open({ tabId: 10, ownership: 'agent' });
  registry.applyEvent(target.targetId, { kind: 'closed', now: 0 });

  const result = registry.applyEvent(target.targetId, { kind: 'updated', patch: { url: 'x' } });
  assert.equal(result, null);
  // The closed target is still gone from the live view.
  assert.equal(registry.byTabId(10), null);
});

test('applying an event to an unknown target returns null', () => {
  const registry = new ManagedTargetRegistry();
  const result = registry.applyEvent('nope', { kind: 'activated' });
  assert.equal(result, null);
});
...
test('auto-generated target ids are deterministic and unique', () => {
  const registry = new ManagedTargetRegistry();
  const first = registry.open({ tabId: 10, ownership: 'agent' });
  const second = registry.open({ tabId: 10, ownership: 'agent' });
  const third = registry.open({ tabId: 12, ownership: 'agent' });

  // Distinct, non-random ids even when the tab id is reused.
  assert.notEqual(first.targetId, second.targetId);
  assert.notEqual(first.targetId, third.targetId);
  assert.notEqual(second.targetId, third.targetId);
  assert.match(second.targetId, /^target-10-\d+$/);
});
