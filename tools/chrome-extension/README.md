...
4. Run `pinpawo-agent browser extension register --extension-id <id>`. Registration preserves previously approved development and Web Store extension IDs.
5. Set `PINPAWO_BROWSER_BACKEND=extension` and start the local agent.

For the official Chrome Web Store build, `pinpawo-agent browser extension register`
